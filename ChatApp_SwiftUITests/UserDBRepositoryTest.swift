...
//
//  Created by Jooeun Kim on 2024-04-30.
//
...
struct StubUserDBReference: DBReferenceType {
    ...
    func setValues(_ values: [String : Any]) -> AnyPublisher<Void, ChatApp_SwiftUI.DBError> {
        Just(()).setFailureType(to: DBError.self).eraseToAnyPublisher()
    }
    ...
    func filter(key: String, path: String?, orderedName: String, queryString: String) -> AnyPublisher<Any?, ChatApp_SwiftUI.DBError> {
    ...
    func observeChildAdded(key: String, path: String?) -> AnyPublisher<Any?, ChatApp_SwiftUI.DBError> {
        Just(nil).setFailureType(to: DBError.self).eraseToAnyPublisher()
    }
